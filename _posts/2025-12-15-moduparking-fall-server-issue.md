...
조회 서버가 분리되면서 독립적인 스케일링이 가능해졌습니다. 그런데 왜 여전히 서버 다운을 막지 못했을까요?
...
조회와 갱신을 분리하려면 캐시가 필수였습니다. 캐시 도입 전, 먼저 현재 병목 지점을 정확히 파악하고자 했습니다.

**clinic.js**로 병목 지점을 분석한 결과, CPU 병목은 없었고 **I/O 큐가 밀리는 것**이 문제였습니다. DB 조회가 많아질수록 I/O 대기 시간이 급격히 증가했습니다.

이 분석 결과를 바탕으로 Redis를 도입하고, 여러 캐시 전략을 조합하여 적용했습니다.
...
유저마다 지도를 조회하는 범위가 각자 다르고, 다양하게 움직이며 조회합니다. 이런 상황에서 캐시 히트률을 높이려면 어떻게 해야 할까요?

> **지오해시(Geohash)란?**
> 위치 좌표를 격자 형태의 문자열로 변환하는 기법입니다. 인접한 위치는 비슷한 문자열을 갖게 되어, 지리적으로 가까운 데이터를 효율적으로 그룹화할 수 있습니다.

지오해시를 활용하면 지도를 일정한 격자로 나눌 수 있습니다. 유저마다 조회 범위가 달라도, 같은 격자에 속하면 동일한 캐시를 재사용할 수 있습니다. 이를 통해 캐시 히트률을 극대화했습니다.
...
# 4. 날짜별 판매량 (STRING) - 핵심!
ticket:soldCount:{ticket_id}:{date}
  → 45
```

**날짜별 판매량 캐시**가 핵심입니다. 자정이 되면 날짜가 바뀌므로, 새로운 캐시 키(`ticket:soldCount:5001:2024-10-06`)를 참조하게 됩니다. 별도의 초기화 작업 없이 자연스럽게 매진 여부가 리셋됩니다.
...
**k6**를 활용해 부하 테스트를 진행하여 성능을 검증했습니다.
